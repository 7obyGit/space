...
describe("TerminalService", () => {
    ...
    it("should run a command and return stdout and stderr", async () => {
        ...
        const mockStderr = "";
        (child_process.exec as any).mockImplementation((command: string, callback: any) => {
        ...
        expect(child_process.exec).toHaveBeenCalledWith("echo 'hello world'", expect.any(Function));
    });
    ...
    it("should return error if exec fails to start", async () => {
        ...
        expect(result.getError()).toContain("Failed to execute command: Spawn failed");
    });
});
